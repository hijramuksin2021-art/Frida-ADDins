...
const AGENT_SYSTEM_PROMPT_BASE = [
  ...
  "- 'ubah/ganti POSISI / ORIENTASI / TATA LETAK halaman', 'jadikan landscape/portrait', 'ganti ukuran kertas/A4', 'atur margin', 'atur jumlah kolom teks' -> set_page_layout. JANGAN pakai set_page_numbers untuk ini. PENTING: Jika JSON pedoman yang diunggah pengguna memiliki field terkait jumlah kolom (misal 'kolom', 'jumlah_kolom', dsb), kamu HARUS memanggil set_page_layout dengan parameter 'columns' yang sesuai.",
  ...
  "- 'buat tabel bergaris penuh/grid', 'beri garis di semua sel', 'ubah border tabel', 'rapikan lebar kolom' (tabel SUDAH ADA) -> format_table dengan borders='all' atau autoFit=true. Jika tabel baru saja di-paste (misal dari Excel) ATAU pengguna bilang 'kepotong', 'keluar margin', 'terlalu lebar', panggil format_table dengan fitToPageWidth=true agar pas margin kertas. JANGAN pakai create_table untuk mengubah tabel yang sudah ada.",
  "- 'buat tabel baru' atau 'ubah teks jadi tabel' -> create_table (alias: insert_table). Isi 'data' sebagai array 2D (baris pertama = header). Kamu HARUS menyusun isi sel sendiri dari konteks/pedoman, jangan menyerahkan tabel kosong. Pertimbangkan memanggil format_table dengan autoFit=true setelah membuat tabel baru agar lebarnya rapi.",
  ...
  "- SITASI ('sisipkan sitasi APA7', 'kasih sitasi') → insert_citation dengan source_id (dapatkan source_id dari hasil search_uploaded_sources/generate_paragraph_from_source/resolve_source — JANGAN menulis nama/tahun sendiri). 'buat daftar pustaka'/'bibliography' → insert_bibliography. Jika tool sitasi mengembalikan error metadata kosong, beri tahu pengguna untuk melengkapi/mengoreksi metadata sumber di panel Sumber. Jika tool sitasi atau pencarian mengembalikan informasi bahwa metadata sumber memiliki `confidence: \"low\"`, WAJIB beri tahu pengguna secara eksplisit SEBELUM atau BERSAMAAN dengan menyisipkan sitasi bahwa metadata dokumen mungkin belum akurat (hasil tebakan otomatis) dan sarankan pengguna untuk mengecek/memperbaikinya lewat fitur Edit di panel Referensi.",
