...
## R7 — Catatan implementasi (Guideline Profile)
Fase ini mengintegrasikan profil institusional (seperti fakultas atau universitas) ke dalam sistem FRIDA, mengatur perilaku RAG (Retrieval-Augmented Generation) maupun gaya penulisan agar sesuai dengan panduan akademik masing-masing prodi.

### Modul & UI Baru
- **`rag/guidelines/`** — Folder yang berisi file konfigurasi `.json` per-fakultas/kampus. Mendefinisikan secara terstruktur: aturan margin & format (kertas, jarak, font), gaya sitasi wajib (mis. APA7), batas plagiarisme, aturan teks khusus, dan struktur dokumen per jenis karya tulis (Skripsi, Usulan Penelitian, PKL).
- **`rag/guidelineConfig.js`** — Manajemen status panduan aktif. Menyimpan profil mana yang sedang aktif secara lokal pada `guideline.local.json` (ter-gitignore). Mengekspos method `status()`, `getActiveId()`, dan `getActiveGuideline()`.
- **`guideline-ui.js`** — Interaksi panel Pengaturan di task pane. Melakukan list API semua file guideline yang tersedia, mempopulerkan dropdown *Profil Fakultas / Institusi*, dan mengeksekusi POST untuk mengubah guideline yang aktif secara real-time.

### Endpoint & Server Integration
- **`GET /api/guidelines` & `/api/guideline`** — Mengembalikan daftar semua opsi profil akademik maupun status aktifnya.
- **`POST /api/guideline`** — Menerima id baru dari UI dan menyimpannya.
- **`getEnforcedStyle()` (server.js)** — Membaca status guideline aktif dan jika gaya sitasi diwajibkan oleh institusi, gaya ini menimpa/memaksa permintaan sitasi untuk tools seperti `/api/sources/cite` dan `/api/sources/bibliography` menjadi gaya tersebut secara otomatis (contoh: memaksa APA7).

### Injeksi Aturan LLM (Prompt Grounded)
- **`rag/generate.js`** (Diperbarui) — `getGroundedSystem()` membaca aturan khusus (misal batas plagiarisme yang didefinisikan kampus, penulisan singkatan/istilah asing, dll) dari profil guideline aktif dan menginjeksinya langsung ke bagian atas *System Prompt* sebelum LLM menghasilkan paragraf. Model menjadi sadar aturan institusi yang ketat ketika memanggil `submit_grounded_paragraph`.

> **STATUS R7: SELESAI.** Profil fakultas sudah terintegrasi dari Task Pane ke Backend dan memengaruhi format LLM serta output sitasi secara deterministik. UI Modern Tabbed Navigation juga telah selesai diterapkan guna memberikan pengalaman layaknya Copilot bawaan Microsoft.